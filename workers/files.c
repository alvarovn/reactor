...
void* scheduler(void *params){
    ...
    serv = (struct rw_services *) params;
    serv->eventhandler("e1");
}

__attribute__ ((visibility("default"))) struct rw_info* rw_init_plugin(const struct rw_services *params){
    int res = 0;
    struct rw_info *info;
    if((info = (struct rw_info *)calloc(1, sizeof(struct rw_info))) == NULL){
        // TODO Log error
        return NULL;
    }
